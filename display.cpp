...
#include "2dNdtLIO(preintegration)/lidar_2d_utils.h"
...
void Display::updateCurrentFrame( std::shared_ptr<Frame> current_frame, bool isKeyframe ) {
...
const cv::Mat Display::getGlobalMap( int max_size ){
    ...
    visualize2DScan(current_frame_->pts_, current_frame_->pose_, display_image, center, resolution );

    // 显示轨迹 TUDO 加上这个画出轨迹之后会存在一点问题
    auto frames = map_->getAllFrames();
    for ( const auto & frame : frames ) {
        Vec2d p_map = frame->pose_.translation() * resolution + center;
        cv::circle(display_image, cv::Point2d(p_map.x(), display_image.rows - p_map.y()), 1, cv::Scalar(0, 0, 255), 1);
    }

    auto voxels = map_->getNdt().getGrid();
    for ( auto voxel : voxels ) {
        Vec2d p_map = Vec2d( voxel.second->second.mu_(0), voxel.second->second.mu_(1) ) * resolution + center;
        cv::circle(display_image, cv::Point2f( p_map.x(), display_image.rows - p_map.y() ), 1, cv::Scalar(229, 0, 0), 1);
    }
    ...
    std::for_each(std::execution::par_unseq, pts.begin(), pts.end(),
        [&](const Vec2d & pt){
            ...
            if (image_x >= 0 && image_x < image.cols && image_y >= 0 && image_y < image.rows) {
                image.at<cv::Vec3b>( image_y, image_x) = cv::Vec3b(0,0,255);  // 红色
            }
        });
